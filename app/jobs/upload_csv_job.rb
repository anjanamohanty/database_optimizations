  ...
  def perform(*args)
    CSV.open(args[0], headers: true) do |c|
      c.each do |line|
        ...
        s = Sequence.find_by(dna: line['Sequence']) || Sequence.create!(assembly_id: a.id, dna: line['Sequence'], quality: line['Sequence Quality'])
        g = Gene.find_by(dna: line["Gene Sequence"]) || Gene.create!(sequence_id: s.id, dna: line["Gene Sequence"], starting_position: line["Gene Starting Position"], direction: line["Gene Direction"])
